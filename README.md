...
[Библиотека] → [Semantic Release] → [npm Registry] → [Renovate] → [Потребители]
│ │ │ │ │
├─ Коммит ├─ Анализирует ├─ Хранит ├─ Видит ├─ Получает
│ feat/fix/! │ Conventional │ версию │ обновление MR с новой
└─ BREAKING │ Commits │ 1.2.3 │ │ версией
├─ Определяет │ ├─ Проверяет └─ Automerge
│ версию │ │ правила после CI
│ (major/minor/patch) │ packageRules
├─ Публикует │ ├─ Создаёт MR
│ в registry │ └─ Ждёт CI
